...
async def predict(img: Image):
    ...
    class_names_list = [classes[int(n)] for n in predictions]

    return encode_image(predicted_img=ploted_image), class_names_list


@app.post('/predict')
async def yolo_model_prediction(image_file_path: Annotated[ImageFilePath, Body(embed=False)]):
    try:
        ...
        prediction_result = await predict(img)
        content = {
